...
// ==========================================
const DOMINIO = 'http://www.forjandomilionarios.com.br'; // MUDE PARA https:// QUANDO TIVER SSL
const PORT = process.env.PORT || 3000; // Railway/Render usa porta dinâmica, local usa 3000

const ADMIN_NUMERO = '5519982020202@c.us';
const ADMIN_SENHA = '22120929';
...
class Database {
    ...
    deletarParticipante(numero) {
        ...
        this.salvar();
        this.log('DELETE', `Participante ${numero} deletado`);
        return { sucesso: true, mensagem: 'Deletado com sucesso' };
    }
    ...
    criarParticipante(dados) {
        ...
        if (this.data.participantes[numero]) {
    ...
    confirmarDoacaoPropria(numero) {
        ...
        this.data.admin.total_arrecadado += VALOR_FASE;
        ...
        this.salvar();
        this.log('DOACAO', `${p.nome} (${numero}) confirmada - R$${VALOR_FASE}`);
    ...
    cadastrarAmigo(numeroP, slot, dadosAmigo) {
        ...
        const numAmigoLimpo = dadosAmigo.numero.replace(/\D/g, '') + '@c.us';
        if (this.data.participantes[numAmigoLimpo]) return { sucesso: false, erro: 'Número já cadastrado no sistema' };
...
client.on('message_create', async (msg) => {
    ...
    try {
        if (isAdmin && textoLower.startsWith('confirmar ')) {
            ...
            const numAlvo = alvo.includes('@c.us') ? alvo : alvo + '@c.us';
            const res = db.confirmarDoacaoPropria(numAlvo);
            
            if (res.sucesso) {
                await msg.reply(`✅ ${res.nome} confirmado! Enviando...`);
                ...
                if (resultadoEnvio.sucesso) {
                    await msg.reply(`✅ Enviado para ${res.nome}`);
                } else {
                    await msg.reply(`⚠️ Erro no envio: ${resultadoEnvio.erro}\nSenha: ${res.senha}`);
                }
            } else {
...
async function iniciarWhatsApp() {
    try {
        ...
        if (erro.message.includes('browser') || erro.message.includes('puppeteer')) {
            console.error('💡 DICA: Verifique se o Chrome está instalado');
        }
    }
}
        ...
        <body>
            ...
            <div class="master-container">
                <div class="epic-content">
                    ...
                    <p class="hero-subtitle">
                        O ecossistema exclusivo onde <span class="highlight">networking de elite</span> encontra 
                        <span class="highlight">multiplicação de capital</span>.
                    </p>
                ...
                <div class="login-section">
                    <div class="login-box">
                        ...
                        <input type="text" id="loginNumero" placeholder="WhatsApp (5511999999999)">
                        ...
                        <button class="btn-secondary" onclick="mostrarCadastro()">
            ...
            <div id="modalCadastro">
                <div class="modal-content">
                    ...
                    <input id="cadEmail" placeholder="Email Profissional" type="email">
                    <input id="cadWhats" placeholder="WhatsApp (5511999999999)">
                    <input id="cadCpf" placeholder="CPF">
                    <input id="cadRg" placeholder="RG">
                    <input id="cadPix" placeholder="Chave PIX Principal">
                    
                    <div class="warning-box">
                        ⚠️ <strong>Importante:</strong> Após o cadastro, realize sua contribuição de R$${VALOR_FASE} para ativação imediata. 
                        Nossa equipe analisará seu perfil e liberará o acesso em até 24h.
                    </div>
                    
                    <button class="btn-primary" onclick="cadastrar()" style="margin-bottom: 0;">
                        Solicitar Admissão
                    </button>
                </div>
            </div>
            ...
            <script>
                ...
                async function cadastrar() {
                    const res = await fetch('/api/cadastrar', {
                        ...
                        body: JSON.stringify({
                            ...
                            whatsapp: document.getElementById('cadWhats').value,
                            cpf: document.getElementById('cadCpf').value,
                            rg: document.getElementById('cadRg').value,
                            pix: document.getElementById('cadPix').value
                        })
                    });
                    const data = await res.json();
                    if(data.sucesso) {
                        alert('✅ Solicitação enviada! Nossa equipe analisará seu perfil e entrará em contato via WhatsApp em breve.');
                ...
                async function login() {
                    const res = await fetch('/api/login', {
                        ...
                        headers: {'Content-Type': 'application/json'},
                        body: JSON.stringify({
                            numero: document.getElementById('loginNumero').value,
            ...
            .map(p => {
                ...
                return `
                    <tr>
                        ...
                        <td>R$ ${(p.fase_atual - 1) * 10}</td>
                        <td>
                            ${podeConfirmar ? `<button class="btn-confirm" onclick="confirmar('${numeroLimpo}', '${p.nome}')">✓ CONFIRMAR R$10</button>` : '<span style="color:#666;">-</span>'}
            ...
            <head>
                ...
                <style>
                    ...
                    * { margin: 0; padding: 0; box-sizing: border-box; }
                    ...
                    .header { background: linear-gradient(135deg, rgba(102,126,234,0.2), rgba(118,75,162,0.2)); border-bottom: 2px solid #667eea; padding: 30px; text-align: center; }
                    .header h1 { font-size: 2.5em; font-weight: 800; background: linear-gradient(135deg, #667eea, #764ba2); -webkit-background-clip: text; -webkit-text-fill-color: transparent; margin-bottom: 10px; }
                    .container { max-width: 1400px; margin: 0 auto; padding: 30px; }
                    .danger-zone { background: rgba(244,67,54,0.1); border: 2px solid #f44336; border-radius: 10px; padding: 15px; margin: 20px 0; text-align: center; }
                    .btn-danger { background: linear-gradient(135deg, #f44336, #d32f2f); border: none; color: #fff; padding: 12px 25px; border-radius: 8px; cursor: pointer; font-weight: 700; }
                    .metas-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(400px, 1fr)); gap: 25px; }
                    .meta-card { background: linear-gradient(135deg, rgba(0,0,0,0.4), rgba(102,126,234,0.1)); border: 2px solid; border-radius: 20px; padding: 30px; }
                    .meta-milhao { border-color: #ffd700; } .meta-dez-milhoes { border-color: #00bcd4; }
                    .progress-container { background: rgba(0,0,0,0.3); border-radius: 15px; height: 30px; overflow: hidden; margin: 15px 0; }
                    .progress-bar { height: 100%; border-radius: 15px; transition: width 0.5s ease; display: flex; align-items: center; justify-content: flex-end; padding-right: 10px; font-weight: 700; font-size: 0.9em; }
                    .meta-milhao .progress-bar { background: linear-gradient(90deg, #ffd700, #ffed4e); color: #000; }
                    .meta-dez-milhoes .progress-bar { background: linear-gradient(90deg, #00bcd4, #00e5ff); color: #000; }
                    .stats-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px; margin: 40px 0; }
                    .stat-card { background: rgba(255,255,255,0.05); border: 1px solid rgba(102,126,234,0.3); border-radius: 16px; padding: 25px; text-align: center; }
                    .stat-value { font-size: 2.2em; font-weight: 800; color: #667eea; }
                    table { width: 100%; border-collapse: collapse; margin-top: 20px; }
                    th { background: rgba(102,126,234,0.2); padding: 15px; text-align: left; color: #667eea; text-transform: uppercase; font-size: 0.9em; }
                    td { padding: 15px; border-bottom: 1px solid rgba(255,255,255,0.05); }
                    .btn-confirm { background: linear-gradient(135deg, #4caf50, #45a049); border: none; color: #fff; padding: 8px 15px; border-radius: 6px; cursor: pointer; font-weight: 600; font-size: 0.8em; }
                    .btn-delete { background: linear-gradient(135deg, #f44336, #d32f2f); border: none; color: #fff; padding: 8px 12px; border-radius: 6px; cursor: pointer; font-size: 0.8em; }
                    .status-badge { padding: 6px 12px; border-radius: 20px; font-size: 0.8em; font-weight: 600; }
                    .status-green { background: rgba(76,175,80,0.2); color: #4caf50; } .status-red { background: rgba(244,67,54,0.2); color: #f44336; } .status-blue { background: rgba(33,150,243,0.2); color: #2196f3; } .status-orange { background: rgba(255,152,0,0.2); color: #ff9800; }
                </style>
            </head>
            <body>
                <div class="header">
                ...
                <div class="container">
                    ...
                    <table>
                        <thead><tr><th>Nome</th><th>WhatsApp</th><th>Status</th><th>Amigos</th><th>Lucro</th><th>Ações</th></tr></thead>
                ...
                <script>
                    function confirmar(numero, nome) {
                        if(confirm('Confirmar doação de R$10 de ' + nome + '?')) {
                            ...
                            .then(r => r.json())
                            .then(data => { 
                                if(data.sucesso) { alert('✅ Confirmado!'); location.reload(); } 
                    ...
                    function deletar(numero, nome) {
                        if(confirm('Deletar ' + nome + ' permanentemente?')) {
                            ...
                            .then(r => r.json())
                            .then(data => { 
                                if(data.sucesso) { alert('🗑️ Deletado!'); location.reload(); } 
                                else { alert('❌ Erro: ' + data.erro); } 
                            });
                        }
                    }
                    ...
                    function zerarTudo() {
                        ...
                        if(senha === '${ADMIN_SENHA}') {
                            if(confirm('ISSO VAI APAGAR TUDO! Tem certeza?')) {
                                ...
                                .then(r => r.json())
                                .then(data => { 
                                    if(data.sucesso) { alert('💥 Sistema zerado!'); location.reload(); } 
                                });
                            }
                        }
                    }
...
appExpress.post('/api/admin/confirmar/:numero', authAdmin, (req, res) => {
    const numero = req.params.numero.includes('@c.us') ? req.params.numero : req.params.numero + '@c.us';
    ...
    if(resultado.sucesso) {
        setImmediate(async () => {
            ...
            await enviarWhatsApp(resultado.numero, mensagem);
        });
        res.json({ sucesso: true, mensagem: 'Confirmado!' });
    } else {
...
appExpress.post('/api/cadastrar', (req, res) => {
    ...
    dados.numero = dados.whatsapp.includes('@c.us') ? dados.whatsapp : dados.whatsapp + '@c.us';
...
appExpress.post('/api/login', (req, res) => {
    ...
    const num = numero.includes('@c.us') ? numero : numero + '@c.us';
    const resultado = db.autenticarUsuario(num, senha);
    res.json(resultado.sucesso ? { sucesso: true, token: resultado.token, dados: resultado.dados } : { erro: resultado.erro });
});
...
appExpress.get('/painel', (req, res) => {
    res.send(`
        ...
        <head>
            ...
            <link rel="icon" href="data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'><text y='.9em' font-size='90'>💰</text></svg>">
            <style>
                body { font-family: 'Segoe UI', sans-serif; background: linear-gradient(135deg, #0f0c29, #302b63); color: #fff; margin: 0; }
                .dashboard { display: grid; grid-template-columns: 250px 1fr; min-height: 100vh; }
                .sidebar { background: rgba(0,0,0,0.3); padding: 30px; border-right: 1px solid rgba(102,126,234,0.2); }
                .main { padding: 40px; }
                .stat-card { background: rgba(255,255,255,0.05); border-radius: 15px; padding: 25px; margin-bottom: 20px; border: 1px solid rgba(102,126,234,0.2); }
                .btn { background: linear-gradient(135deg, #667eea, #764ba2); border: none; color: #fff; padding: 15px 30px; border-radius: 10px; cursor: pointer; font-weight: 600; margin-right: 10px; margin-bottom: 10px; }
                .btn:disabled { opacity: 0.5; cursor: not-allowed; }
                .amigo-card { background: rgba(255,255,255,0.03); border-radius: 10px; padding: 20px; margin: 10px 0; border: 1px solid rgba(102,126,234,0.2); }
                ...
                @media (max-width: 768px) { .dashboard { grid-template-columns: 1fr; } .sidebar { border-right: none; border-bottom: 1px solid rgba(102,126,234,0.2); } }
            </style>
        </head>
        <body>
            <div class="dashboard">
                ...
                <div class="main">
                    ...
                    <div class="stat-card" style="background: rgba(255,215,0,0.05); border-color: #ffd700;">
                        ...
                        <p style="color: rgba(255,255,255,0.6);">Lucro acumulado por fase completada</p>
                    </div>
                </div>
            </div>
            ...
            <script>
                ...
                async function carregar() {
                    try {
                        ...
                        document.getElementById('saldo').textContent = ((data.dados.fase - 1) * 10).toFixed(2).replace('.', ',');
                        ...
                        if(!podeCadastrar) {
                            document.getElementById('btnCadastrar').title = 'Aguarde a confirmação da doação';
                        }
                        ...
                        if(!data.dados.amigos_cadastrados.amigo_1 && !data.dados.amigos_cadastrados.amigo_2) {
                            html = '<p style="color: rgba(255,255,255,0.5);">Nenhum amigo cadastrado.</p>';
                        }
                ...
                function cadastrarAmigo() {
                    ...
                    if(!nome) return;
                    const numero = prompt('WhatsApp (5511999999999):');
                    if(!numero) return;
                    
                    fetch('/api/cadastrar-amigo', {
                        method: 'POST',
                        headers: {'Content-Type': 'application/json', 'Authorization': 'Bearer '+token},
                        body: JSON.stringify({slot: 'amigo_1', nome, numero})
                    })
                    .then(r => r.json())
                    .then(data => {
                        if(data.sucesso) {
...
appExpress.post('/api/cadastrar-amigo', (req, res) => {
    ...
    if(resultado.sucesso) {
        setImmediate(async () => {
            const mensagem = 
                ...
                `🔑 *Senha:* ${resultado.senha}\n\n` +
                `🌐 *Acesse:* ${DOMINIO}/painel\n\n` +
                `⚠️ Faça sua doação de R$${VALOR_FASE}!\n\n` +
    ...
    res.json(resultado.sucesso ? {sucesso: true, senha: resultado.senha} : {erro: resultado.erro});
});

appExpress.get('/qr', async (req, res) => {
    if(!qrCodeAtual) return res.send('<h2 style="color:green; text-align:center; margin-top:50px;">✅ WhatsApp já conectado!</h2>');
    try {
        const url = await QRCodeLib.toDataURL(qrCodeAtual);
        res.send('<html><head><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Forjando Milionários - QR Code</title></head><body style="display:flex;justify-content:center;align-items:center;height:100vh;background:#1a1a2e; margin:0;"><div style="text-align:center;"><h2 style="color:#667eea;margin-bottom:20px;">Escaneie com o WhatsApp</h2><img src="' + url + '" style="width:300px;height:300px;"><p style="color:#fff;margin-top:20px;">Abra o WhatsApp → Configurações → Aparelhos Conectados</p></div></body></html>');
    } catch(e) {
